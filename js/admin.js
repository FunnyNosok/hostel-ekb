...
document.getElementById('hostel-form').addEventListener('submit', (e) => {
  ...
  const existing = isEdit ? list[index] : null;

  // В форме только три поля для фото — остальные снимки галереи оставляем как есть.
  const photos = [];
  for (let i = 0; i < 3; i++) {
    const src = document.getElementById(`h-photo${i}`).value.trim();
    if (src) photos.push(src);
  }
  if (existing && existing.photos) photos.push(...existing.photos.slice(3));

  // Форма правит лишь часть полей. Остальные (street, area, type, specs)
  // переносим из текущей записи, иначе лендинг потеряет их без следа.
  const hostel = { ...(existing || {}), id, name, address, price, description, features, photos };
